...
- 功能新增：**无限画布**接入 Home 主导航，支持图层管理、灵感库、动态模型参数、资源库选图、放大/抠图/反推与任务占位轮询 🔥🔥🔥
- 功能新增：AI 绘图接入 **GPT Image 2.5 Flare / Sunburst**，参数与 API 兼容 GPT Image 2
- 功能新增：**提示词库**支持预览
- 功能优化：用户端采用分区紫罗兰主题，管理后台绿色主题隔离不变
- 功能优化：资源列表与资源库缩略图按存储引擎自动选型，画布选区支持删除与点选不置顶
- 功能优化：统一集成测试门禁与 Playwright 静态路由巡检，隔离测试库与 Redis DB
- 功能重构：**算力日志**与**推广计划**改为独立路由页，不再使用 Home 内嵌弹窗
- Bug修复：**Suno** 查询拆单与列表轮询，避免 downloading 状态指数复制任务；管理端批量删除与退款逻辑修正
- Bug修复：**画布生图**远程图片转存超时与重试，下载中状态可持续轮询并正确回填图层
- Bug修复：**PPT 配图**按真 16:9 尺寸出图，预览不再因 3:2 比例左右留白
- Bug修复：画布跨域 OSS 图经鉴权代理加载，图层缩略图与导出倍率显示正常


## v4.3.5

- 功能新增：**数据库维护**支持纯 Go 备份恢复与版本迁移一体页，升级后 Dashboard 引导执行迁移，安装脚本同步提醒 🔥🔥🔥
- 功能新增：**我的资源**网盘化，支持文件夹、搜索、移动、预览与登录下载，资源弹窗与缩略图体验统一 🔥🔥🔥
- 功能新增：接入 **Seedance** 独立视频渠道，并支持 **Seedream 5.0 Pro** 与 **qwen-image-3.0-pro** 文生图/图生图 🔥🔥🔥
- 功能新增：**AI 绘图/视频展示配置**，管理端可拖拽排序与启用开关，用户端按配置过滤，提交时拒绝已禁用项
- 功能新增：**AIGC 任务准入与并发调度**，按用户限制非终态任务配额，五类提交并发可配置，降低排队与超卖风险
- 功能优化：**PPT** 导出缓存与进度、风格锁定、SSE 草稿流及创建页交互
- 功能优化：**AIGC 创作页**统一 Image/Video/Suno/MJ/即梦布局、任务轮询、失败提示与重启退款兜底
- 功能优化：**管理后台配置页**密钥字段默认掩码显示，演示与运维时降低明文泄露风险
- 功能优化：统一 **JWT 唯一会话**与登录限流，支付结算原子化；AIGC 后台任务接入可停止的 BackgroundGroup
- 功能优化：拆分 **Chat / MJ / PPT** 页面为组件与 composable，补齐 ESLint 与 Quality CI
- 功能优化：**品牌首屏**按授权状态占位加载，避免 Logo 闪烁；快乐小马默认升级为 1.1
- 功能重构：移除 **desktop** 与 **miniprogram** 目录及打包入口，收敛为 Web 主线交付
- 功能重构：移除用户端 **TaskList** 与算力不足 WebSocket 通知，失败提示改走页面内统一交互
- Bug修复：**AIGC 任务失败**自动退款并统一 `refunded_at` 幂等兜底，覆盖 MJ/生图/Suno/视频/即梦
- Bug修复：**本地存储**资源库图片 URL 解析错误，历史 OSS 地址可正确映射为可访问路径
- Bug修复：**Logo 尺寸**、首页/登录页 Footer 贴底、验证码弹窗与对话侧栏布局
- Bug修复：**即梦**任务状态展示与视频预览弹窗尺寸；万相无参考图文生图避免编辑模式 400
- Bug修复：**管理后台登录页**页脚文字改为白色，暗色背景上可读

## v4.3.4

- 功能新增：**管理后台数据迁移**支持 SSE 五步迁移向导，自动对齐 Schema、清理废弃表与多余列，并移除服务启动时的自动迁移，运维升级更可控
- 功能新增：**管理后台数据库备份**支持异步全库导出与 SSE 逐表进度展示，迁移页一键备份并自动下载 SQL 文件
- 功能新增：**任务失败重试与退款追踪**，生图、视频、Suno、MJ、即梦等任务支持失败后重新入队，并记录退款时间便于核对算力退还
- 功能新增：**算力不足通知**通过 WebSocket 实时推送，避免用户在长时间任务中才发现余额不足
- 功能优化：**即梦任务提交**对齐 `price_key` 计价键，提示词输入组件重构，复杂任务扣费更准确
- 功能重构：**人机验证**改为 go-captcha 本地生成与 Redis 校验，不再依赖外部验证码 API Key
- 功能重构：**聊天模型权限**移除开放标记、标签字段与用户级模型白名单，启用模型对所有用户可见，简化配置与迁移逻辑
- 功能优化：**管理后台配置页**重构基础、插件、算力等设置布局；示例数据库配置密钥统一脱敏，降低开源仓库泄露风险
- Bug修复：**生产构建**撤销将 Vue 与 Element Plus 拆开的 `manualChunks`，恢复整包 Element Plus，消除线上 `Cannot access before initialization` 白屏
- Bug修复：**对话侧栏**头像尺寸与列表布局；`chat-plus.scss` 改为非 scoped 引入，并在 `ChatSidebar` 内约束头像
- Bug修复：**菜单缓存**对非数组 `data` 做防护，避免无后端预览时首屏崩溃
- Bug修复：**视频任务轮询**在查询失败或空队列时继续等待，避免后台同步提前空转退出

## v4.3.3

- 功能新增：**视频生成**新增 Vidu、Happyhorse 等多厂商适配器，并统一 Relay、DashScope 公共客户端，视频创作可接入更多模型服务
- 功能新增：**生图管理后台**新增图片配置与任务管理页面，支持后台查看、配置和删除生图任务，降低运营排查成本
- 功能优化：**即梦 AI**支持多维度 `price_key` 算力定价，前后端按模型、能力和参数精确计算积分，避免复杂任务计费不一致
- 功能优化：**视频与生图创作**统一参数配置、算力计算和移动端体验，任务创建、扣费和展示逻辑更一致
- 功能优化：**任务管理**支持失败视频任务重新入队，已完成或失败任务可删除，用户和管理员处理异常任务更方便
- 功能优化：**PPT 生成**补强幻灯片蒙版编辑、参考图和配图流程，提升演示文稿生成后的可编辑性
- 功能重构：清理历史归档 SQL 文件，保留当前 `geekai_plus.sql` 作为数据库基线，减少部署和维护时的版本干扰
- Bug修复：移除**提示词生成接口**的 30 秒限流，避免连续生成提示词时被误拦截
